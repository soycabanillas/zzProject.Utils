...
#pragma warning restore 0219
        ...
        Expression ParseAggregate(Expression instance, Type elementType, string methodName, int errorPos)
        {
            ...
            if (FindMethod(typeof(IEnumerableSignatures), methodName, false, args, out signature) != 1)
                throw ParseError(errorPos, SR.NoApplicableAggregate(methodName));
            Type[] typeArgs;
        ...
        Expression[] ParseArgumentList()
        {
            ...
            ValidateToken(TokenId.CloseParen, SR.CloseParenOrCommaExpected);
            NextToken();
            return args;
        }
        ...
        {
            int errorPos = token.pos;
            ValidateToken(TokenId.OpenBracket, SR.OpenParenExpected);
            NextToken();
            Expression[] args = ParseArguments();
            ValidateToken(TokenId.CloseBracket, SR.CloseBracketOrCommaExpected);
            NextToken();
            if (expr.Type.IsArray)
            {
                if (expr.Type.GetArrayRank() != 1 || args.Length != 1)
                    throw ParseError(errorPos, SR.CannotIndexMultiDimArray);
                Expression index = PromoteExpression(args[0], typeof(int), true);
                if (index == null)
                    throw ParseError(errorPos, SR.InvalidIndex);
            ...
            else
            {
                ...
                {
                    case 0:
                        throw ParseError(errorPos, SR.NoApplicableIndexer(
                            GetTypeName(expr.Type)));
                    case 1:
                        return Expression.Call(expr, (MethodInfo)mb, args);
                    default:
                        throw ParseError(errorPos, SR.AmbiguousIndexerInvocation(
                            GetTypeName(expr.Type)));
                }
            }
        }
        ...
        void CheckAndPromoteOperand(Type signatures, string opName, ref Expression expr, int errorPos)
        {
            ...
            if (FindMethod(signatures, "F", false, args, out method) != 1)
                throw ParseError(errorPos, SR.IncompatibleOperand(
        ...
        {
            return ParseError(pos, SR.IncompatibleOperands(
            ...
            switch (ch)
            {
                ...
                case '\'':
                    ...
                    do
                    {
                        ...
                        if (textPos == textLen)
                            throw ParseError(textPos, SR.UnterminatedStringLiteral);
                        NextChar();
                    } while (ch == quote);
                ...
                default:
                    ...
                    throw ParseError(textPos, SR.InvalidCharacter(ch));
            }
            token.id = t;
        ...
        {
            ValidateToken(TokenId.Identifier, SR.IdentifierExpected);
        ...
        {
            if (!Char.IsDigit(ch))
                throw ParseError(textPos, SR.DigitExpected);
        }
        ...
        {
            if (token.id != t)
                throw ParseError(SR.SyntaxError);
        }
